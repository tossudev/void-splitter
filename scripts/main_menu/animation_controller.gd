...
const ANIM_VALUES_TITLE: Dictionary = {
	...
	"rotation_time" : 0.875,		# Time multiplier for animation
...
func do_object_animation(
	...
	# If changing scene, skip animations.
...
func _on_button_up(button_node: Object):
	Audio.play_effect("res://assets/sound/effects/ui_select.wav")
	
	# If the mouse has left the button,
	# revert back to original size instead of scaled up size.
	if !mouse_on_object:
		do_object_animation(
			"scale",
			button_node,
