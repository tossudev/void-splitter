...
func _ready() -> void:
	# Disable and gray out the Quit button in web builds
	if OS.has_feature("HTML5") or OS.has_feature("web"):
		$Menus/Main/Buttons/ButtonQuit.set_modulate(Color.DIM_GRAY)
		$Menus/Main/Buttons/ButtonQuit.set_disabled(true)
